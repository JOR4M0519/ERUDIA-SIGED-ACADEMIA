...
class UserAdapterTest {
    ...
    @Test
    void registerByGroupinStudentUser_nullUserDomain_throwsBadRequest() {
        UserRegistrationDomain reg = UserRegistrationDomain.builder().build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerByGroupinStudentUser(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }

    @Test
    void registerByGroupinStudentUser_nullDetail_throwsBadRequest() {
        UserDomain ud = UserDomain.builder().build();
        UserRegistrationDomain reg = UserRegistrationDomain.builder().userDomain(ud).build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerByGroupinStudentUser(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }

    @Test
    void registerByGroupinStudentUser_nullGroupId_throwsBadRequest() {
        UserDomain ud = UserDomain.builder().build();
        UserDetailDomain dd = UserDetailDomain.builder().build();
        UserRegistrationDomain reg = UserRegistrationDomain.builder().userDomain(ud).userDetailDomain(dd).build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerByGroupinStudentUser(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }

    @Test
    void registerAdministrativeUsers_nullUserDomain_throwsBadRequest() {
        UserRegistrationDomain reg = UserRegistrationDomain.builder().build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerAdministrativeUsers(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }

    @Test
    void registerAdministrativeUsers_nullDetail_throwsBadRequest() {
        UserDomain ud = UserDomain.builder().build();
        UserRegistrationDomain reg = UserRegistrationDomain.builder().userDomain(ud).build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerAdministrativeUsers(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    ...
    @Test
    void registerAdministrativeUsers_emptyRoles_throwsBadRequest() {
        UserDomain ud = UserDomain.builder().roles(Set.of()).build();
        UserDetailDomain dd = UserDetailDomain.builder().build();
        UserRegistrationDomain reg = UserRegistrationDomain.builder().userDomain(ud).userDetailDomain(dd).build();
        AppException ex = assertThrows(AppException.class, () -> adapter.registerAdministrativeUsers(reg));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    ...
    @Test
    void patchGeneralUser_nullUserId_throwsBadRequest() {
        AppException ex = assertThrows(AppException.class, () -> adapter.patchGeneralUser(null, null));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }

    @Test
    void patchGeneralUser_nullDomain_throwsBadRequest() {
        AppException ex = assertThrows(AppException.class, () -> adapter.patchGeneralUser(1, null));
        assertEquals(HttpStatus.BAD_REQUEST, ex.getCode());
    }
}